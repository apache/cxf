...
public class DynamicClientFactory {
        ...
        public Client createClient(String wsdlUrl, QName service,
                ...
                WSDLServiceFactory sf = (service == null)
                    ? (new WSDLServiceFactory(bus, wsdlUrl)) :
                    (new WSDLServiceFactory(bus, wsdlUrl, service));
                ...
                if (!src.mkdir()) {
                        throw new IllegalStateException(
                            "Unable to create working directory " + src.getPath());
                }
                try {
                ...
                if (!classes.mkdir()) {
                        throw new IllegalStateException(
                            "Unable to create working directory " + classes.getPath());
                }
                ...
                if (srcFiles.size() > 0 && !compileJavaSrc(classPath.toString(), srcFiles,
                                                           classes.toString())) {
                ...
                try {
                        ...
                        throw new IllegalStateException(
                            "Internal error; a directory returns a malformed URL: "
                            + mue.getMessage(), mue);
                }
                ...
                for (ServiceInfo si : serviceList) {
                        for (SchemaInfo sci : si.getSchemas()) {
                                ...
                                if (ele != null) {
                                        ...
                                        opts.addGrammar(is);
                                        schemaCompiler
                                            .parseSchema(key, StaxUtils.createXMLStreamReader(ele, key));
                                }
                        }
                }
                ...
                try {
                        ...
                        if (resolver.isResolved()) {
                                ...
                                throw new ServiceConstructionException(
                                    new Message("COULD_NOT_RESOLVE_URL", LOG, s));
                        }
                } catch (IOException e) {
        ...
        class InnerErrorListener {
                ...
                public void error(SAXParseException arg0) {
                        ...
                        if (errors.length() == 0) {
                                errors.append("Error compiling schema from WSDL at {").append(url)
                                    .append("}: \n");
                        } else {
                                errors.append("\n");
                        }
        ...
        private void hackInNewInternalizationLogic(SchemaCompiler schemaCompiler,
                ...
                Object o = ((ReflectionInvokationHandler)Proxy.getInvocationHandler(schemaCompiler))
                    .getTarget();
                try {
                ...
                if (catalog.hasCatalogs()) {
                        try {
                                ...
                                if (!xil.getClass().getName().contains(".internal.")) {
                                        ...
                                        LOG.warning(
                                            "Cannot set a catalog resolver into the JDK internal XJC compiler.  Catalog"
                                            + " resolved schemas may not work correctly");
                                }
                        } catch (Throwable ex) {
                                LOG.log(Level.WARNING,
                                        "Cannot set a catalog resolver into the XJC compiler.  Catalog"
                ...
                try {
                        return new InternalizationLogic() {
                                ...
                                public boolean checkIfValidTargetNode(DOMForest parent, Element bindings,
                                                                      Element target) {
                                        return ((InternalizationLogic)xil)
                                            .checkIfValidTargetNode(parent, bindings, target);
                                }
        ...
        public Node cloneNode(Document document, Node node, boolean deep) throws DOMException {
                ...
                case Node.ELEMENT_NODE:
                        ...
                        for (int i = 0; i < attributes.getLength(); i++) {
                                ((Element)clone)
                                    .setAttribute(attributes.item(i).getNodeName(), attributes.item(i)
                                        .getNodeValue());
                        }
                        break;
        ...
        public class LocationFilterReader extends StreamReaderDelegate {
                ...
                private String mapSchemaLocation(String target) {
                        return DynamicClientFactory
                            .mapSchemaLocation(target, this.getLocation().getSystemId(), catalog);
                }
