...
public class ClientImpl
    ...
    implements Client, Retryable, MessageObserver {
        ...
        protected Map<String, Object> currentRequestContext = new ConcurrentHashMap<String, Object>(8, 0.75f,
                                                                                                    4);
        ...
        public ClientImpl(Bus bus, Service svc, QName port,
                ...
                try {
                        if (endpointImplFactory != null) {
                                getConduitSelector()
                                    .setEndpoint(endpointImplFactory.newEndpointImpl(bus, svc, epfo));
                        } else {
                                getConduitSelector().setEndpoint(new EndpointImpl(bus, svc, epfo));
                        }
                } catch (EndpointException epex) {
                        throw new IllegalStateException("Unable to create endpoint: " + epex.getMessage(),
                                                        epex);
                }
                notifyLifecycleManager();
        }
                                        ...
                                        if ("http://schemas.xmlsoap.org/wsdl/soap/".equals(bid)
                                            || "http://schemas.xmlsoap.org/wsdl/soap12/".equals(bid)) {
                                                for (Object o : bfo.getExtensors().get()) {
                                                        try {
                                                                String s = (String)o.getClass()
                                                                    .getMethod("getTransportURI").invoke(o);
        ...
        public Map<String, Object> getRequestContext() {
                if (isThreadLocalRequestContext()) {
                        if (!requestContext.containsKey(Thread.currentThread())) {
                                requestContext
                                    .put(Thread.currentThread(), new EchoContext(currentRequestContext));
                        }
        ...
        public Object[] invoke(BindingOperationInfo oi,
                ...
                try {
                        ...
                        if (context != null) {
                                Map<String, Object> resp = CastUtils
                                    .cast((Map<?, ?>)context.get(RESPONSE_CONTEXT));
        ...
        private Object[] doInvoke(final ClientCallback callback,
                ...
                try {
                        ...
                        if (LOG.isLoggable(Level.FINE)) {
                                LOG.fine(
                                    "Invoke, operation info: " + oi + ", params: " + Arrays.toString(params));
                        }
                                        ...
                                        public void onMessage(Message message) {
                                                ...
                                                if (ex != null) {
                                                        ...
                                                        if (message.getContent(Exception.class) == null) {
                                                                ...
                                                                Message inMsg = message.getExchange()
                                                                    .getInMessage();
                                                                Map<String, Object> ctx = responseContext
                                                                    .get(Thread.currentThread());
                                                                resList = CastUtils
                                                                    .cast(inMsg.getContent(List.class));
                ...
                synchronized (exchange) {
                        ...
                        if (!Boolean.TRUE.equals(exchange.get(FINISHED))) {
                                LogUtils.log(LOG, Level.WARNING, "RESPONSE_TIMEOUT",
                                             exchange.getBindingOperationInfo().getOperationInfo().getName()
                                                 .toString());
