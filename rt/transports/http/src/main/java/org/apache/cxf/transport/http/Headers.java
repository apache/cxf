...
public class Headers {
     ...
     * enabled.
     */
    static String toString(Map<String, List<String>> headers) {
    ...
    */
     ...
     */
    static void logProtocolHeaders(Logger logger, Level level, Map<String, List<String>> headersMap) {
        if (logger.isLoggable(level)) {
            for (Map.Entry<String, List<String>> entry : headersMap.entrySet()) {
                String key = entry.getKey();
                boolean sensitive = SENSITIVE_HEADERS.contains(key);
    ...
    public void setProtocolHeadersInConnection(HttpURLConnection connection) throws IOException {
        ...
        logProtocolHeaders(LOG, Level.FINE, headers);
    }
