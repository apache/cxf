...
public class OIDCFlowTest extends AbstractBusClientServerTestBase {
    ...
    public void testAuthorizationCodeFlow() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address, OAuth2TestUtils.setupProviders(),
                                            "alice", "security", null)) {

            // Save the Cookie for the second request...
            WebClient.getConfig(mainClient).getRequestContext().put(
                org.apache.cxf.message.Message.MAINTAIN_SESSION, Boolean.TRUE);
    
            // Get Authorization Code
            String code = OAuth2TestUtils.getAuthorizationCode(mainClient, "openid");
            assertNotNull(code);
    
            // Now get the access token
            try (WebClient client = WebClient.create(address, "consumer-id", "this-is-a-secret", null)) {
    ...
    public void testAuthorizationCodeFlowPOST() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address, OAuth2TestUtils.setupProviders(),
            ...
            mainClient.type("application/x-www-form-urlencoded");
    
            mainClient.path("authorize/");
            ...
            Response response = mainClient.post(form);
    
            OAuthAuthorizationData authzData = response.readEntity(OAuthAuthorizationData.class);
            String location = OAuth2TestUtils.getLocation(mainClient, authzData, null);
    ...
    public void testAuthorizationCodeOAuth() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address,  OAuth2TestUtils.setupProviders(),
            ...
            // Get Authorization Code
            String code = OAuth2TestUtils.getAuthorizationCode(mainClient, "read_balance");
            assertNotNull(code);
    ...
    public void testAuthorizationCodeFlowWithNonce() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address,  OAuth2TestUtils.setupProviders(),
            ...
            // Get Authorization Code
            String code = OAuth2TestUtils.getAuthorizationCode(mainClient, "openid", "consumer-id",
                                                               "123456789", null);
            assertNotNull(code);
    ...
    public void testAuthorizationCodeFlowWithScope() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address,  OAuth2TestUtils.setupProviders(),
            ...
            // Get Authorization Code
            String code = OAuth2TestUtils.getAuthorizationCode(mainClient, "openid read_balance");
            assertNotNull(code);
    ...
    public void testAuthorizationCodeFlowWithRefresh() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address, OAuth2TestUtils.setupProviders(),
            ...
            // Now get the access token
            try (WebClient tokenClient = WebClient.create(address, "consumer-id", "this-is-a-secret", null)) {

                ClientAccessToken accessToken =
                    OAuth2TestUtils.getAccessTokenWithAuthorizationCode(tokenClient, code);
                assertNotNull(accessToken.getTokenKey());
                assertTrue(accessToken.getApprovedScope().contains("openid"));
                ...
                try (WebClient client = WebClient.create(address, OAuth2TestUtils.setupProviders(),
    ...
    public void testAuthorizationCodeFlowWithAudience() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address,  OAuth2TestUtils.setupProviders(),
            ...
            // Get Authorization Code
            String code = OAuth2TestUtils.getAuthorizationCode(mainClient, "openid", "consumer-id-aud",
                                                               null, null);
            assertNotNull(code);
    
            // Now get the access token
            try (WebClient client = WebClient.create(address, "consumer-id-aud", "this-is-a-secret", null)) {
    ...
    public void testAuthorizationCodeFlowWithPKCE() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address,  OAuth2TestUtils.setupProviders(),
            ...
            String location = OAuth2TestUtils.getLocation(mainClient, parameters);
            String code = OAuth2TestUtils.getSubstring(location, "code");
            ...
            try (WebClient client = WebClient.create(address, "consumer-id", "this-is-a-secret", null)) {
                ClientAccessToken accessToken =
                    OAuth2TestUtils.getAccessTokenWithAuthorizationCode(client, code, "consumer-id", 
                          null, codeVerifier);
                assertNotNull(accessToken.getTokenKey());
    ...
    public void testHybridCodeIdToken() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address, OAuth2TestUtils.setupProviders(),
                                            "alice", "security", null)) {
            WebClient.getConfig(mainClient).getHttpConduit().getClient().setReceiveTimeout(100000000);
            // Save the Cookie for the second request...
            WebClient.getConfig(mainClient).getRequestContext().put(
                org.apache.cxf.message.Message.MAINTAIN_SESSION, Boolean.TRUE);
    ...
    public void testAuthorizationCodeFlowRefreshToken() throws Exception {
        ...
        try (WebClient mainClient = WebClient.create(address, OAuth2TestUtils.setupProviders(),
            ...
            // Get Authorization Code
            String code = OAuth2TestUtils.getAuthorizationCode(mainClient,
                String.join(" ", OidcUtils.getOpenIdScope(), OAuthConstants.REFRESH_TOKEN_SCOPE),
                "consumer-id-oidc");
            assertNotNull(code);
    
            // Now get the access token
            try (WebClient client = WebClient.create(address, "consumer-id-oidc", "this-is-a-secret", null)) {
    ...
    private static IdToken getIdToken(ClientAccessToken accessToken, String jwksUri, String clientId) {
        try (WebClient c = WebClient.create(jwksUri,
            ...
            // See please https://issues.apache.org/jira/browse/CXF-9161
            WebClient.getConfig(c).getRequestContext().put(
                "share.httpclient.http.conduit", Boolean.FALSE);
