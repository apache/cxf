...
public class FiqlParserReDoSTest {
    ...
    @Test
    public void testDefaultExpressionLengthLimitIsFourKiB() throws SearchParseException {
        FiqlParser<Bean> parser = new FiqlParser<>(Bean.class);
        parser.parse(expressionOfLength(4096));
        try {
            parser.parse(expressionOfLength(4097));
        } catch (SearchParseException ex) {
            return;
        }
        throw new AssertionError("The default expression length limit must be 4 KiB");
    }
